...
void        operation_zjmp(t_carriage *self)
{
    ...
    op.args[ARG_1] = arena_get_n_bytes_from(self->arena, self->arena_position + OP_CODE_SIZE, op.t_dir_size);
    if (self->carry == 1)
        self->arena_position = (self->arena_position + op.args[ARG_1]  % IDX_MOD) % MEM_SIZE;
    else
