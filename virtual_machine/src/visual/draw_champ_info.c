...
void		draw_champ_info(t_vm *vm)
{
	...
	{
		wattron(vm_get_wins(vm)->champ, COLOR_PAIR(P_1_HOME + i) | A_BOLD);
		wmove(vm_get_wins(vm)->champ, i * 3 + 1, 1);
		wprintw(vm_get_wins(vm)->champ, "P_%d_name: %40s", i + 1,
			(tmp = player_get_text_name(arena_get_player(vm_get_arena(vm), i))));
		ft_memdel((void **)&tmp);
		wmove(vm_get_wins(vm)->champ, i * 3 + 2, 1);
		wprintw(vm_get_wins(vm)->champ, "P_%d_comment: %37s", i + 1,
			player_get_text_comment(arena_get_player(vm_get_arena(vm), i)));
		ft_memdel((void **)&tmp);
		wmove(vm_get_wins(vm)->champ, i * 3 + 3, 1);
		wprintw(vm_get_wins(vm)->champ, "carriage_num: %36d",
			arena_get_carriage_num(vm_get_arena(vm), i));
		wattroff(vm_get_wins(vm)->champ, COLOR_PAIR(P_1_HOME + i) | A_BOLD);
		++i;
	}
}
